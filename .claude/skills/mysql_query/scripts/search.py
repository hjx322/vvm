...
def _mask_id(value: Any, keep_head: int = 2, keep_tail: int = 2) -> str:
    """病历号等患者标识符的日志掩码（Fix: 日志明文打印 PHI）

    日志文件同样属于数据落盘范围，明文病历号会让日志系统本身成为 PHI 泄露源。
    保留首尾少量字符以便排查（"1881921" → "18***21"），过短的标识整体打码。

    注意：**只用于日志**。函数返回值（返回给用户的查询结果）不做掩码——
    用户本来就有权看到自己的病历号。
    """
...
def search(input) -> str:
    ...
    try:
        ...
        # Fix: 日志中的病历号统一掩码；返回给用户的结果不做掩码（见 _mask_id 说明）
        logger.info(f"Executing MySQL query: 表名={db_name}, 病历号={_mask_id(medical_record_no)}, CRM={crm}")
        ...
        with get_crm_db(crm) as patient_db:
            if db_name == "patient_info":
                ...
                if patient_info:
                    logger.info(f"成功查询患者信息: {_mask_id(medical_record_no)}")
                    return patient_info.to_str()
                else:
                    logger.warning(f"未找到患者信息: {_mask_id(medical_record_no)}")
                ...
                if visit_record_list:
                    logger.info(f"成功查询患者就诊记录: {_mask_id(medical_record_no)}，共 {len(visit_record_list)} 条")
                    return "\n".join([item.to_str() for item in visit_record_list])
                else:
                    logger.warning(f"未找到患者就诊记录: {_mask_id(medical_record_no)}")
                ...
                if examine_result_list:
                    logger.info(f"成功查询患者检查结果: {_mask_id(medical_record_no)}，共 {len(examine_result_list)} 条")
                    return "\n".join([item.to_str() for item in examine_result_list])
                else:
                    logger.warning(f"未找到患者检查结果: {_mask_id(medical_record_no)}")
