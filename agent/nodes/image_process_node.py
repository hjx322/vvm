...
class ImageProcessNode:
    ...
    # ===== 确认/否定词表（Fix: 用户回"不对/不是"会被子串匹配误判为确认）=====
    # 否定词必须优先判定：否则"不对"含"对"、"不是"含"是"，会被当成"同意"直接执行检测。
    NEGATION_WORDS = (
        "不", "别", "否", "无", "取消", "算了", "重新", "换一个", "换张",
    )
    # 肯定词；短答（≤6 字）走精确匹配，避免"对"命中"对不起"这类无关表述。
    CONFIRM_WORDS = (
        "是", "是的", "对", "对的", "没错", "可以", "好", "好的",
        "行", "行的", "同意", "确认", "嗯",
    )
    _SHORT_REPLY_MAX = 6  # 短答长度阈值（字符）
...
请告诉我您想对这张图片做什么？"""
    ...
    async def _prepare_skill_execution(
        ...
        Returns:
            更新后的状态字典（包含 sub_agent_input，供 orchestrator 图片快路径执行 derma_image）
        """
...
正在处理..."""
        ...
        # 将图片路径写入 sub_agent_input，供 orchestrator 图片快路径调用 derma_image skill
        # 设置 sub_agent_input 为包含图片路径的 JSON，orchestrator 会提取使用
        ...
        return {
            ...
            "image_pending_skill": None,  # 已进入执行，清掉暂存的待确认技能
            "messages": new_messages,
            "sub_agent_input": skill_input,  # 供 orchestrator 图片快路径提取
        }
...
这个判断准确吗？"""
        ...
        return {
            ...
            "image_pending_skill": skill_name,  # 暂存待确认技能，用户确认后直接复用
            "messages": new_messages,
        }
    ...
    async def _handle_skill_confirmation(
        ...
        confirmed = self._is_confirmed(human_input)

        if confirmed:
            # 用户确认，优先复用反问时暂存的技能（Fix: 原来此处重跑一次 LLM 意图识别，
            # 属纯冗余——同一张图同一段描述，结果必然相同，且 LLM 非确定性还会导致漂移）
            image_description = state.get("image_description", "")
            skill_name = state.get("image_pending_skill")

            if skill_name and self.check_skill_exists(skill_name):
                logger.info(f"用户确认技能（复用暂存）: {skill_name}")
                return await self._prepare_skill_execution(
                    skill_name, image_path, image_description, state
                )

            # 兜底：旧 checkpoint 无 image_pending_skill 字段时，退回 LLM 识别
            logger.info("无暂存技能（兼容旧会话），降级为 LLM 识别")
            ...
            return {
                ...
                "image_description": state.get("image_description", ""),
                "image_processing_status": "waiting_for_user_intent",
                "image_pending_skill": None,  # 用户否决，清掉待确认技能
                "messages": state.get("messages", []) + [
    ...
    @classmethod
    def _is_confirmed(cls, human_input: str) -> bool:
        """判断用户是否给出肯定确认（否定优先 + 短答精确匹配）

        原实现 `any(word in text for word in ["是","对",...])` 有两个误判：
          ① "不对" 含 "对"、"不是" 含 "是" → 否定被当成同意（直接执行检测）；
          ② "对不起，换一个" 同理误命中。
        现改为：先判否定词（命中即否决），再按回答长度分流——短答要求精确等于肯定词，
        长句才用包含匹配（"是的，帮我检测一下"仍可命中）。矛盾表述（"不，是的"）按否决
        处理并回到等待选择，属安全侧兜底。
        ...
        Returns:
            True=确认，False=未确认（含否定与无法判断）
        """
        text = (human_input or "").strip().lower()
        if not text:
            return False

        # ① 否定优先
        if any(w in text for w in cls.NEGATION_WORDS):
            return False

        # ② 肯定：短答精确匹配，长句包含匹配
        if len(text) <= cls._SHORT_REPLY_MAX:
            return text in cls.CONFIRM_WORDS
        return any(w in text for w in cls.CONFIRM_WORDS)
