...
class SkillManager:
    ...
    def upload_skill(
        ...
        """
        上传或更新技能。元数据优先级（与下方代码一致）：
        1. 用户显式传入的参数（**优先级最高**，用于覆盖 SKILL.md）
        2. SKILL.md frontmatter 中的值
        3. 默认值（language='python3'）

        注：`content` 是例外——**总是取 SKILL.md 正文**，不接受外部覆盖。

        Fix: 原 docstring 写成"1. frontmatter 最优先 / 2. 用户参数覆盖 SKILL.md"，
        两句自相矛盾；代码实际是 `skill_id or skill_meta.get(...)`，即显式参数优先。
        ...
        if existing_skill:
            ...
            # 为该租户的所有 worker（多 agent 下 agents 表当作 worker 注册表）创建技能关联
            # （is_enabled=False，由前端在「Agent」页手动为某 worker 启用），与顶部注释对齐
